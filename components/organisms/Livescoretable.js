...
const Table = ({ columns, data }) => {
  ...
  return (
    <>
      <table {...getTableProps()} className=" mx-auto mt-14 w-full font-light xl:w-2/3  ">
        <thead>
          {headerGroups.map((headerGroup) => (
            <tr {...headerGroup.getHeaderGroupProps()} className="border ">
              {headerGroup.headers.map((column) => (
                <th {...column.getHeaderProps()}>
                  {column.render('Header') == 'League' ? (
                    <div>
                      {column.canFilter ? (
                        <div>
                          {column.render('Filter')}{' '}
                          <span className="relative bottom-7 "> {column.render('Header')}</span>
                        </div>
                      ) : null}
                    </div>
                  ) : (
