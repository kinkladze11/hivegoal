...
const Subscription = () => {
  ...
  const active = user?.premium && new Date().getTime() < new Date(user?.premium).getTime()
  return (
    <Layout>
      <div className="mx-auto my-5 w-full bg-[url('/foo.jpg')] bg-cover py-5 text-center lg:w-2/3">
        <h2 className="mt-10 text-3xl text-white">WIN PREDICTIONS WITH PREMIUM TOOLS</h2>
        {active && (
        ...
        {!active && (
          <div className={'mt-8 text-2xl font-bold text-white'}>No Active membership</div>
        )}
        <div className={'my-9 mx-8 flex flex-wrap justify-center gap-8 md:flex-nowrap'}>
